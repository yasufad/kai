...
problems encountered while using Kai.
...
### How do I update Kai to the latest version?
...
- **Use a `.env` file:** Create a `.env` file in your project's `.gemini`
  directory (`.gemini/.env`) and store your keys there. Kai will automatically
...
### Where are the Kai configuration and settings files stored?

The Kai configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.gemini/settings.json`.
2.  In your project's root directory: `./.gemini/settings.json`.

Refer to [Kai Configuration](./get-started/configuration.md) for more details.
...
Search the
[Kai Q&A discussions on GitHub](https://github.com/google-gemini/gemini-cli/discussions/categories/q-a)
or
