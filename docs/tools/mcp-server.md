...
- **Access resources:** Read data from specific resources (though the Kai
  primarily focuses on tool execution).

With an MCP server, you can extend the Kai's capabilities to perform actions
beyond its built-in features, such as interacting with databases, APIs, custom
scripts, or specialized workflows.

## Core Integration Architecture

The Kai integrates with MCP servers through a sophisticated discovery and
...
The Kai supports three MCP transport types:
...
The Kai uses the `mcpServers` configuration in your `settings.json` file to
...
The Kai supports OAuth 2.0 authentication for remote MCP servers using SSE or
...
When the Kai starts, it performs MCP server discovery through the following
detailed process:
...
Kai's capabilities while maintaining security, reliability, and ease of use.
...
The Kai will correctly process this array, separating text from binary data and
...
When the Kai receives this response, it will:
...
executed as slash commands within the Kai. This allows you to create shortcuts
...
```bash
/poem-writer --title="Kai" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Kai" reverent
```

When you run this command, the Kai executes the `prompts/get` method on the MCP
...
`settings.json` file, the Kai provides a convenient set of commands to manage
