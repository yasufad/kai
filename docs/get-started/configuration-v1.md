...
- **`coreTools`** (array of strings):
  - **Description:** Allows you to specify a list of core tool names that should
    ...
    be executed.
  - **Default:** All tools available for use by the Kai model.
...
- **`allowMCPServers`** (array of strings):
  ...
  - **Default:** All MCP servers are available for use by the Kai model.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](../cli/themes.md) for Kai.
  - **Default:** `"Default"`
...
- **`mcpServers`** (object):
  - **Description:** Configures connections to one or more Model-Context
    ...
    compatibility. At least one of `command`, `url`, or `httpUrl` must be
    provided. If multiple are specified, the order of precedence is `httpUrl`,
