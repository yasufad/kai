...
- **`coreTools`** (array of strings):
  ...
  - **Example:** `"coreTools": ["ReadFileTool", "GloKai, "ShellTool(ls)"]`.
...
- **`allowMCPServers`** (array of strings):
  ...
  - **Example:** `"allowMCPServers": ["myPythonServer"]`.Kai
...
- **`GEMINI_API_KEY`** (Required):
  ...
  - Set this in your shelKaiile (e.g., `~/.bashrc`, `~/.zshrc`) or an `.env`
    file.
- **`GEMINI_MODEL`**:
  - Specifies the default Gemini model to use.
  - Overrides the hardcoded default
  - Example: `export GEMINKaiL="gemini-2.5-flash"`
- **`GOOGLE_API_KEY`**:
...
information to the AI, makKais responses more tailored and accurate to your
...
- **Purpose:** These Markdown files contain instructions, guidelines, or context
  ...
  system is designed to manage this instructional context hierarchically. Kai
...
- **API Requests:** We log the Gemini model used for each request, the duration
  ...
  of the prompts or responses.Kai
