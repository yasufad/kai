...
1.  **Global context file:**
    - **Location:** `~/.gemini/KAI.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `KAI.md` file in your current working
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `KAI.md` files in subdirectories
...
Here is an example of what you can include in a `KAI.md` file at the root of a
TypeScript project:
...
- **`/memory refresh`**: Forces a re-scan and reload of all `KAI.md` files from
  all configured locations.
- **`/memory add <text>`**: Appends your text to your global `~/.gemini/KAI.md`
  file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `KAI.md` files into smaller, more manageable components
by importing content from other files using the `@file.md` syntax. This feature
supports both relative and absolute paths.

**Example `KAI.md` with imports:**
...
While `KAI.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "KAI.md"]
  }
}
```
