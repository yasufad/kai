...
To use Kai tools, provide a prompt to the Kai. The process works as follows:

1.  You provide a prompt to the Kai.
...
in mind. The Kai will typically:
...
- **Utilize sandboxing:** All tools are subject to restrictions enforced by
  sandboxing (see [Sandboxing in the Kai](../cli/sandbox.md)). This means that
  when operating in a sandbox, any tools (including MCP servers) you wish to use
  must be available _inside_ the sandbox environment. For example, to run an MCP
  server through `npx`, the `npx` executable must be installed within the
...
## Learn more about Kai's tools

Kai's built-in tools can be broadly categorized as follows:
