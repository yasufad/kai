...
prompts as personal shortcuts within Kai. You can create commands that are
...
Kai discovers commands from two locations, loaded in a specific order:
...
When a custom command attempts to execute a shell command, Kai will now prompt
...
Kai will then execute the multi-line prompt defined in your TOML file.
