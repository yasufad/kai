...
Kai uses JSON settings files for persistent configuration. There are four
...
- **User settings file:**
  ...
  - **Scope:** Applies to all Kai sessions for the current user. User settings
...
- **System settings file:**
  ...
  - **Scope:** Applies to all Kai sessions on the system, for all users. System
    ...
    May be useful for system administrators at enterprises to have controls over
    users' Kai setups.
...
- **`ui.showStatusInTitle`** (boolean):
  - **Description:** Show Kai status and thoughts in the terminal window title
  - **Default:** `false`
...
- **`tools.enableHooks`** (boolean):
  - **Description:** Enable the hooks system for intercepting and customizing
    Kai behavior. When enabled, hooks configured in settings will execute at
    appropriate lifecycle events (BeforeTool, AfterTool, BeforeModel, etc.).
...
discovering and using custom tools. Kai attempts to connect to each configured
...
tool definitions for compatibility. At least one of `command`, `url`, or
`httpUrl` must be provided. If multiple are specified, the order of precedence
is `httpUrl`, then `url`, then `command`.
...
Configures logging and metrics collection for Kai. For more information, see
