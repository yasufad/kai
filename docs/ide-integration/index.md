...
Kai can integrate with your IDE to provide a more seamless and context-aware
...
- **VS Code Commands:** You can access Kai features directly from the VS Code
  Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Kai: Run`: Starts a new Kai session in the integrated terminal.
  - `Kai: Accept Diff`: Accepts the changes in the active diff editor.
  - `Kai: Close Diff Editor`: Rejects the changes and closes the active diff
    editor.
  - `Kai: View Third-Party Notices`: Displays the third-party notices for the
    extension.
...
When you run Kai inside a supported editor, it will automatically detect your
...
manually, you can run the following command inside Kai:
...
> NOTE: The "Kai Companion" extension may appear towards the bottom of search
...
When enabled, Kai will automatically attempt to connect to the IDE companion
extension.
...
- Open the Command Palette and run **Kai: Accept Diff**.
...
- Open the Command Palette and run **Kai: Close Diff Editor**.
...
If you are using Kai within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker Container:** If you run Kai inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** Kai could not find the necessary environment variables
  ...
  - **Solution:**
    1.  Make sure you have installed the **Kai Companion** extension in your IDE
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. Kai is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Kai in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Kai in a terminal or environment that is not a
    supported IDE.
  - **Solution:** Run Kai from the integrated terminal of a supported IDE, like
    VS Code.

- **Message:**
  `No installer is available for IDE. Please install the Kai Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Kai
    Companion", and
